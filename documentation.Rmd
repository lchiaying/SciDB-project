...
**Charlie: Add explanation of data structures**
...
We installed the SciDB from scratch.

**Charlie: add installation instructions**
...
**Chia: add explanation of data, describe clustering goal**
...
```{r find_size_small_dataset, eval = F}
...
iquery(dbConnect, 
       "store(redimension(ratings, <rating:double NOT NULL>[userId=0:671,?,0,movieID=0:163949,?,0]), ratings_matrix)")
...
```{r count_small_dataset, eval = F, cache = T}
...
Similarly, we can obtain the the average average rating of each movie.

```{r}
movieAvg <- iquery(dbConnect, "aggregate(ratings_matrix, avg(rating), movieID)", return = T)
head(movieAvg)
```
...
1. Compute the SVD of the ratings matrix and keep the top 10 singular values. Apply kmeans clustering on the top 10 singular component coefficients.

The correlation matrix and SVD will be computed using SciDB functionality, while kmeans and hierarchical clustering will be computed using base R functionality.
...
We visualize the correlation matrix using the `heatmap` function in R. (Actually, we'll visualize `1-correlationMatrix`, which is the dissimilarity matrix.) This function also automatically computes the dendrogram and reorders the rows and columns to produce a visual clustering of the userIds.
...
Next, we perform the SVD on the ratings matrix to obtain the top princple components of the ratings data.
...
query_userSum <- "aggregate(ratings_matrix, sum(rating), movieID)"
query_crossjoin <- sprintf("cross_join(ratings_matrix as A, %s as B, A.movieID, B.movieID)", 
                           query_userSum)
query_center <- sprintf("project(apply(%s, rating_centered, rating-rating_sum/671.0), rating_centered)", 
                        query_crossjoin)
...
By inspecting the plot of the singular values, we see that the top 3 singular values capture the most variation in the data.
...
We also need the left and right singular vectors. We'll get SciDB to compute and store the singular vectors, and then extract only the first 10 singular vectors into R.

```{r gesvd_U_VT_small_data, eval = F, cache = T}
ratingsSvdU <- iquery(dbConnect, "store(gesvd(ratings_matrix_centered, 'U'), ratingsSvdU)", return = T)
ratingsSvdU[[1]]=as.integer(ratingsSvdU[[1]])
ratingsSvdU[[2]]=as.integer(ratingsSvdU[[2]])
save(ratingsSvdU,file="/home/scidb/Project/ratingsSvdU.Rdata")

ratingsSvdVt <- iquery(dbConnect, "store(gesvd(ratings_matrix_centered, 'VT'), ratingsSvdVt)", return = T)
save(ratingsSvdVt,file="/home/scidb/Project/ratingsSvdVt.Rdata")
```


```{r gesvd_UxSdiag_small_data, eval = F}
query <- "apply(apply(ratings_singularValues, i_2, i_1-1), j, i_2)"  # Please check if i or i_1 is the dimension name, and whether it starts at 0 or 1?
query <- sprintf("redimension(%s, <sigma:double>[i_2=0:671,?,0,j=0:671,?,0])", query) 
iquery(dbConnect, sprintf("store(%s, ratingsSvdSdiag)", query))
iquery(dbConnect, "store(spgemm(ratingsSvdU, ratingsSvdSdiag), ratingsSvdUxS)")
```


```{r PCcoords, cache = T}
load("/home/scidb/Project/ratingsSvdU.Rdata")

uMat <- spMatrix(nrow = 672, ncol = 672, 
                 i = ratingsSvdU$userId + 1, 
                 j = ratingsSvdU$i + 1, 
                 x = ratingsSvdU$u)
sigMat <- Diagonal(x = ratings_singularValues$sigma)
# The +1 is due to difference in 0-indexing in SciDB and 1-indexing in R

PCcoords <- uMat * sigMat

# ratings_U10 <- iquery(dbConnect, "subarray(ratings_U, null, 0, null, 4)", return = T)
# ratings_VT10 <- iquery(dbConnect, "subarray(ratings_VT, null, 0, null, 4)", return = T)

```

```{r}
plot(PCcoords[, 1], PCcoords[, 2])
plot(PCcoords[, 2], PCcoords[, 3])
plot(PCcoords[, 2], PCcoords[, 4])
plot(PCcoords[, 4], PCcoords[, 5])
```
...
# Big Data Computations with the Full Dataset

The computations in the previous section extend to using the full dataset. However, due to limited RAM and hard disk space on our laptops, we are unable to run the identical computations for the full dataset.

We can still show a tractable example: finding the pairwise correlations between movies.


```{r spgemm_full_data, eval=F}
system.time(
  iquery(dbConnect, 
         "store(spgemm(transpose(RATINGS_MATRIX), RATINGS_MATRIX), RATINGS_MATRIX_MxMT)")
)
```
