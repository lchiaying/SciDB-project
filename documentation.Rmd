...
query <- sprintf("redimension(%s, <sigma:double NOT NULL>[i_2=0:671,?,0,j=0:671,?,0])", query) 
...
uMat <- spMatrix(nrow = 672, ncol = 672, 
                 ...
                 j = ratingsSvdU$i + 1, 
                 x = ratingsSvdU$u)
sigMat <- Diagonal(x = c(0,ratings_singularValues$sigma))
# The +1 is due to difference in 0-indexing in SciDB and 1-indexing in R

PCcoords <- uMat * sigMat[1:(nrow(sigMat)-1), 1:(nrow(sigMat)-1)]
