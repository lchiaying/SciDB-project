...
```{r show_ratings_matrix_MxMT_10x10, cache = T}
ratings_matrix_MxMT_10 <- iquery(dbConnect, "subarray(ratings_matrix_MxMT, 0, 0, 9, 9)", return = T)
(Matrix::spMatrix(nrow = 10, ncol = 10, 
                  i = ratings_matrix_MxMT_10$userId, 
                  j = ratings_matrix_MxMT_10$userId2,
                  x = ratings_matrix_MxMT_10$multiply))
```

### Hierarchical clustering

```{r, eval = T, cache = T}
ratings_matrix_MxMT <- iquery(dbConnect, "subarray(ratings_matrix_MxMT, null, null, null, null)", return = T)

library(Matrix, quietly = T)
correlationMatrix <- spMatrix(nrow = 671, ncol = 671, 
                              i = ratings_matrix_MxMT$userId, 
                              j = ratings_matrix_MxMT$userId2,
                              x = ratings_matrix_MxMT$multiply)

# hc <- hclust(as.dist(correlationMatrix))
```

## Clustering using Principle Component Analysis and the SVD

Next, we perform the SVD on the ratings matrix to obtain the top 10 princple components of the ratings data.
...
**Charlie to run this chunk**

```{r gesvd_small_data, eval = F, cache = T}
iquery(dbConnect, "store(gesvd(ratings_matrix, 'S'), ratings_singularValues)")
```

By inspecting the plot of the singular values, we see that the 10 singular values capture a large amount of the variation in the data.

```{r plot_singularValues, eval = T, cache = T}
# ratings_singularValues <- iquery(dbConnect, "subarray(ratings_singularValues, null, null)", return = T)
# plot(ratings_singularValues)
```


We also need the left and right singular values. We'll get SciDB to compute and store the singular values, and then extract only the first 10 singular values into R.

**Charlie to run this chunk**
```{r gesvd_U_VT_small_data, eval = F, cache = T}
iquery(dbConnect, "store(gesvd(ratings_matrix, 'U'), ratings_U)")
iquery(dbConnect, "store(gesvd(ratings_matrix, 'VT'), ratings_VT)")
```


```{r gesvd_U_VT_get10_small_data, cache = T}
# ratings_U10<- iquery(dbConnect, "subarray(ratings_U, null, 0, null, 9)", return = T)
# ratings_VT10<- iquery(dbConnect, "subarray(ratings_VT, null, 0, null, 9)", return = T)
```
